...
import { ChatProvider } from "@/components/realtime/chat-provider"
...
export default async function RootLayout({
  ...
  if (user) {
    try {
      ...
      initialUnreadMessages = messageUnreadCount
    } catch (error) {
    ...
    <html lang="en">
      <body className={`${geistSans.variable} ${geistMono.variable} bg-[#fafafa] font-sans text-foreground antialiased`}>
        <NotificationsProvider initialUnreadCount={initialUnreadCount} sessionUserId={user?.id ?? null}>
          <ChatProvider initialUnreadCount={initialUnreadMessages} sessionUserId={user?.id ?? null}>
